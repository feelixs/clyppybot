...
from bot.tools import create_nexus_str, reply_or_dm
...
class KickAutoEmbed(Extension):
    ...
    async def _process_this_clip_link(self, clip_link: str, respond_to: Message, include_link=False):
        ...
        if parsed_id in self.too_large_clips:
            ...
            await reply_or_dm(self.bot, respond_to, embed=emb)
            self.logger.info(
        ...
        if clip_file is None:
            ...
            emb.description += create_nexus_str()
            await reply_or_dm(self.bot, respond_to, embed=emb, delete_after=60)
            return 1
        try:
            if include_link:

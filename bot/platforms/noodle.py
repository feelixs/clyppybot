...
class NoodleMisc(BaseMisc):
    def __init__(self):
        super().__init__()
        self.platform_name = "Noodle"
        self.is_nsfw = True
        self.dl_timeout_secs = 120

    def parse_clip_url(self, url: str, extended_url_formats=False) -> Optional[str]:
        pattern = r'(?:https?://)?(?:www\.)?noodlemagazine.com/watch/([a-zA-Z0-9_-]+)'
    ...
    async def get_clip(self, url: str, extended_url_formats=False, basemsg=None, cookies=True) -> 'NoodleClip':
        ...
        return NoodleClip(shortcode)


class NoodleClip(BaseClip):
    def __init__(self, shortcode):
        self._service = "noodle"
